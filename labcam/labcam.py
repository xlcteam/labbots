...
class AV(ToxAV):
    ...
    def on_call(self, friend_number, audio_enabled, video_enabled):
        print("Incoming call: %d, %d, %d" % (friend_number,
                                             audio_enabled,
                                             video_enabled))
        self.answer(friend_number, 8, 500)
        print("Answered, in call...")
        self.on_start(friend_number)

    def on_call_state(self, friend_number, state):
        print('call state:%d,%d' % (friend_number, state))
        if state == 2:
            self.on_end(friend_number)
        ...
        while not self.stop:
            try:
                ...
                if ret:
                    ...
                    self.video_send_frame(idx, width, height, frame.tostring())
...
class Phone(Tox):
    def __init__(self, opts=None):
        ...
        self.self_set_name("XLC Lab Cam")
